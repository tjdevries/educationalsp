...
`
	encoded := golsp.EncodeMessage(msg)

	decoded, err := golsp.DecodeMessage(encoded)
	if err != nil {
		t.Fatal(err)
	}

	if decoded.(golsp.InitializeMessage).Method != "initialize" {
		t.Fatalf("\n%+v\n", decoded)
	}
}
...
func TestRealDecode(t *testing.T) {
	...
	decoded, err := golsp.DecodeMessage(msg)
	if err != nil {
		t.Fatal(err)
	}
